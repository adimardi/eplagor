...
class DataspanestimasipnbpImport implements ToModel, WithHeadingRow, WithChunkReading, ShouldQueue, WithEvents
{
    /**
    * @param array $row
    *
    ...
    */
    ...
    public function model(array $row)
    {
        if($row['baes1'] == '005'.$this->eselon)
        {
            ...
            return dataspan_estimasipnbp::updateOrCreate([
                'id' => $this->tahun.".".$row['kdsatker'].".".$row['baes1'].".".$row['akun']]
            ,[
                'thang' => $this->tahun,
    ...
    public function registerEvents(): array
    {
        return [
            ...
            BeforeImport::class => function(BeforeImport $event) {
                $eselon = $this->eselon;
                $updateImport = dataspan_estimasipnbp::query()
                                                        ->with('reffsatker')
            ...
            AfterImport::class => function(AfterImport $event) {
                $eselon = $this->eselon;
                $hapusdataspan = dataspan_estimasipnbp::with('reffsatker')
                                                        ->whereHas('reffsatker', function($q) use($eselon) {
                                                            $q->where('kode_eselon', $eselon)
                                                                ;})
                                                        ->where('thang', Session::get('thang'))
