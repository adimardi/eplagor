...
class DataspanrealisasipnbpImport implements ToModel, WithHeadingRow, WithChunkReading, ShouldQueue, WithEvents
{
    /**
    * @param array $row
    *
    ...
    */
    ...
    public function model(array $row)
    {
        if($row['baes1'] == '005'.$this->eselon)
        {
            ...
            return dataspan_realisasipnbp::updateOrCreate([
                //
                'id' => $this->tahun.".".$row['kdsatker'].".".$row['baes1'].".".$row['akun'].".".date("Y-m-d", strtotime(str_replace('/','-',$row['tanggal']))).".".$row['amount']]
            ,[
                'dataspan_estimasipnbp_id' => $this->tahun.".".$row['kdsatker'].".".$row['baes1'].".".$row['akun'],
                'thang' => $this->tahun,
    ...
    public function registerEvents(): array
    {
        return [
            ...
            BeforeImport::class => function(BeforeImport $event) {
                $eselon = $this->eselon;
                $updateImport = dataspan_realisasipnbp::query()->with('reffsatker')
            ...
            AfterImport::class => function(AfterImport $event) {
                $eselon = $this->eselon;
                $hapusdataspan = dataspan_realisasipnbp::with('reffsatker')
                                                        ->whereHas('reffsatker', function($q) use($eselon) {
                                                            $q->where('kode_eselon', $eselon)
                                                                ;})
                                                        ->where('thang', Session::get('thang'))
