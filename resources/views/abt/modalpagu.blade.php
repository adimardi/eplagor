...
<script type="text/javascript">
    window.livewire.on('usersStored', () => {
        $('#tablePagux').DataTable().ajax.reload();
    });
</script>
...
<script type="text/javascript">
    ...
    $('#tablePagux thead tr').clone(true).appendTo( '#tablePagux thead' );
    $('#tablePagux thead tr:eq(1) th').each( function (i) {
        var title = $(this).text();
    ...
    var table = $('#tablePagux')
        .DataTable({

        processing: true,
        serverSide: true,
        ajax: {
            "url"  : "{{ route ('api.paguabt') }}", 
            "data" : function (d) {
...
<div class="table-responsive p-3">
    <table id="tablePagux" class="table table-resposive table-sm order-column nowrap align-items-center mb-0" width="100%">
        <thead>
            <tr>
