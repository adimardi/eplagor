        ...
        var table = $('#tableBaseline')
      	.DataTable({
          	...
          	columns: [
                      	...
                      	{ data: 'pagu_id', sClass: "text-secondary mb-0 text-center", width: "50px"},
                      	...
                      	{ data: 'uraian_subkomponen', sClass: "text-secondary mb-0 text-left"},
      	...
      	$(document).on('click','.btn-upload',function()
      	{
	        ...
	        //form.find('input[name="id"]').val(rowData.reffsatker_id+rowData.kode_program+rowData.kode_kegiatan+rowData.kode_output+rowData.kode_suboutput+rowData.kode_komponen)
	        form.find('input[name="id"]').val(rowData.pagu_id)
	        form.find('input[name="kodeSatker"]').val(rowData.reffsatker_id)
	        //form.find('input[name="kodeUnit"]').val(rowData.kdunit)
	        form.find('input[name="kodeProgram"]').val(rowData.kode_program)
	        form.find('input[name="kodeKegiatan"]').val(rowData.kode_kegiatan)
	        form.find('input[name="kodeOutput"]').val(rowData.kode_output)
	        form.find('input[name="kodeSubOutput"]').val(rowData.kode_suboutput)
	        form.find('input[name="kodeKomponen"]').val(rowData.kode_komponen)
	      	...
	      	<div class="card my-4">
	        	<div class="card-header p-0 position-relative mt-n4 mx-3 z-index-2">
	          		<div class="bg-gradient-primary shadow-primary border-radius-lg pt-4 pb-3">
	            		<h6 class="text-white text-capitalize ps-4">Daftar Data Dukung {{ $config['pageTitle'] }}</h6>
	          		</div>
	        	</div>
				            	...
				            	<table id="tableBaseline" class="table table-bordered table-resposive table-sm order-column nowrap align-items-center mb-0" width="100%">
				              		<thead>
				                		<tr>
				                  			...
				                  			<th class="text-uppercase text-secondary font-weight-bolder">Id Pagu</th>
...
<div class="modal bs-upload-modal-md" id="mUpload" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  	<div class="modal-dialog" style="width: 1024px;">
    	<div class="modal-content">
      		...
      		<form class="form" action="{{ route('paguindikatif.uploads') }}" method="POST" enctype="multipart/form-data">
